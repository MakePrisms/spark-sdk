...
fn package_wasm_target(
    ...
    // Remove .gitignore generated by wasm-pack — it contains `*` which causes
    // npm to exclude all files in the directory even when listed in `files`.
...
/// Generate an ESM wrapper (`index.mjs`) for the Node.js CJS entry point.
///
/// Scans the wasm-bindgen generated `breez_sdk_spark_wasm.js` for
/// `module.exports.X = ...` patterns and creates a named re-export for each
/// public symbol, so `import { connect } from '@agicash/breez-sdk-spark'`
/// works in ESM contexts. The CJS `index.js` re-exports that file via
/// `module.exports = require('./breez_sdk_spark_wasm.js')`, so all exports
/// are available on the default import.
fn generate_esm_wrapper(out_path: &Path) -> Result<()> {
    let wasm_js_path = out_path.join("breez_sdk_spark_wasm.js");
    let wasm_js_content = fs::read_to_string(&wasm_js_path)
        .with_context(|| format!("Failed to read {}", wasm_js_path.display()))?;
    ...
    let destructured = exports.join(",\n  ");
    let mjs_content = format!(
        "// ESM wrapper for the CJS Node.js entry point.\n\
         // Auto-generated by `cargo xtask package` — do not edit manually.\n\
         import pkg from './index.js';\n\
         \n\
         export const {{\n  {destructured},\n}} = pkg;\n\
         \n\
    ...
    let mjs_path = out_path.join("index.mjs");
    fs::write(&mjs_path, mjs_content)
        .with_context(|| format!("Failed to write {}", mjs_path.display()))?;

    println!(
        "Generated ESM wrapper at {} ({} exports)",
        mjs_path.display(),
        exports.len()
    );
    Ok(())
